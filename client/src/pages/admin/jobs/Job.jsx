      ...
      <main className="flex-1 p-4 border-t bg-gray-100/50">
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {repairs.length > 0 ? (
              repairs.map((booking) => <Stepper key={booking.id} booking={booking} />)
            ) : (
              <p className="text-sm italic text-gray-600">No repair jobs found.</p>
            )}
          </div>
        </section>
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {consultations.length > 0 ? (
              consultations.map((booking) => <Stepper key={booking.id} booking={booking} />)
            ) : (
              <p className="text-sm italic text-gray-600">No consultation jobs found.</p>
            )}
          </div>
        </section>
