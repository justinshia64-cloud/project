...
export async function deletePart(req, res) {
  try {
    const { id } = req.params
    const part = await prisma.part.findUnique({ where: { id: parseInt(id) }, include: { partsUsed: true, logs: true } })
    if (!part) return res.status(404).json({ message: 'Part not found' })

    // Prevent deletion if part has been used in jobs or inventory logs exist
    if ((part.partsUsed || []).length > 0 || (part.logs || []).length > 0) {
      return res.status(400).json({ message: 'Cannot delete part with usage or logs. Consider archiving instead.' })
    }
